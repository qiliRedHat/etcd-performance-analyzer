...
class nodeAnalyzerMCPAgent:
    ...
    async def _initialize_node(self, state: AgentState) -> AgentState:
        ...
        stream_print("\n" + "="*100)
        stream_print("🚀 INITIALIZING NODE PERFORMANCE ANALYSIS SESSION")
        stream_print("="*100)
    ...
    async def _collect_metrics_node(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if metrics_data and metrics_data.get("status") == "success":
                ...
                stream_print(" ✅ Done")
                stream_print("\n📦 Metrics Summary:")
                stream_print(f"  • Master Node cpu metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('master',[]).get('metrics',[]).get('cpu_usage',{}).get('nodes',[]))} records")
                stream_print(f"  • Master Node memory metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('master',[]).get('metrics',[]).get('memory_used',{}).get('nodes',[]))} records")
                stream_print(f"  • Master Node cgroup cpu metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('master',[]).get('metrics',[]).get('cgroup_cpu_usage',{}).get('nodes',[]))} records")
                stream_print(f"  • Master Node cgroup rss metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('master',[]).get('metrics',[]).get('cgroup_rss_usage',{}).get('nodes',[]))} records")
                stream_print(f"  • Worker Node cpu metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('worker',[]).get('metrics',[]).get('cpu_usage',{}).get('nodes',[]))} records")
                stream_print(f"  • Worker Node memory metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('worker',[]).get('metrics',[]).get('memory_used',{}).get('nodes',[]))} records")
                stream_print(f"  • Worker Node cgroup cpu metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('worker',[]).get('metrics',[]).get('cgroup_cpu_usage',{}).get('nodes',[]))} records")
                stream_print(f"  • Worker Node cgroup rss metrics: {len(data.get('node_usage_data', []).get('node_groups',[]).get('worker',[]).get('metrics',[]).get('cgroup_rss_usage',{}).get('nodes',[]))} records")
...
async def main():
    """Main function"""
    stream_print("\n" + "="*100)
    stream_print("🔧 NODE PERFORMANCE ANALYZER")
    stream_print("="*100)
